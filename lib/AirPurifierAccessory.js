 ...
 * - showChildLockSwitch -   boolean for adding a visible Switch tile for child lock. Default is false.
 * - nameChildLock -         Allows customisation of the child lock switch name. Default is 'Child Lock'
 *
...
class AirPurifierAccessory extends BaseAccessory {
        ...
        switch (this.device.context.manufacturer) {
            ...
            default: // Just use numeric values
                ...
                for (let i = 0; i <= fanSpeedSteps; i++) {
                    _fanSpeedLabels[i] = this.device.context.stringSpeedValues ? String(i) : i;
                }
        }
        this._rotationStops = {0: _fanSpeedLabels[0]};
        for (let i = 0; i < 100; i++) {
            const _rotationStep = Math.round(fanSpeedSteps * i / 100);
    ...
    _registerPlatformAccessory() {
        ...
        /* If configured to show a child lock switch, add that service too */
        if (this.device.context.showChildLockSwitch) {
            this._addChildLockSwitchService();
        }
    ...
    _addChildLockSwitchService() {
        const {Service} = this.hap;
        const nameChildLock = this.device.context.nameChildLock || 'Child Lock';
        this.log.info('Adding child lock switch: %s', nameChildLock);
        this.accessory.addService(Service.Switch, nameChildLock);
    }
    /**
    ...
    _registerCharacteristics(dps) {
        ...
        /* Child lock Switch service */
        let characteristicChildLockSwitch;
        let childLockSwitchService = this.accessory.getService(Service.Switch);
        if (this.device.context.showChildLockSwitch) {
            if (!childLockSwitchService) {
                this._addChildLockSwitchService();
                childLockSwitchService = this.accessory.getService(Service.Switch);
            }
            const nameChildLock = this.device.context.nameChildLock || 'Child Lock';
            this._checkServiceName(childLockSwitchService, nameChildLock);
            characteristicChildLockSwitch = childLockSwitchService.getCharacteristic(Characteristic.On)
                .updateValue(!!dps[DP_LOCK_PHYSICAL_CONTROLS])
                .on('get', (callback) => {
                ...
                .on('set', (value, callback) => {
                    this.setState(DP_LOCK_PHYSICAL_CONTROLS, !!value, callback);
                });
        } else if (childLockSwitchService) {
            this.accessory.removeService(childLockSwitchService);
        }
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty(DP_LOCK_PHYSICAL_CONTROLS)) {
                /* Child Lock change */
                if (characteristicLockPhysicalControls) {
                ...
                if (characteristicChildLockSwitch) {
                    const newChildLockSwitch = !!changes[DP_LOCK_PHYSICAL_CONTROLS];
                    if (characteristicChildLockSwitch.value !== newChildLockSwitch) {
                        characteristicChildLockSwitch.updateValue(newChildLockSwitch);
                    }
                }
            }
            if (changes.hasOwnProperty(DP_MODE)) {
    ...
    setRotationSpeed(value, callback) {
        ...
        if (value === 0) {
            ...
            // Send the new speed AND ensure the device is powered on, so users can
            // turn the purifier on by adjusting the speed in HomeKit.
            const newState = {
                [DP_SWITCH]: true,
                [DP_FAN_SPEED]: this.convertRotationSpeedFromHomeKitToTuya(value),
            };
            this.log.debug('setRotationSpeed value: %s. State: %o', value, newState);
    ...
    convertRotationSpeedFromHomeKitToTuya(value) {
        const tuyaSpeed = this._rotationStops[parseInt(value)];
        this.log.debug('convertRotationSpeedFromHomeKitToTuya: %s → %s', value, tuyaSpeed);
        return tuyaSpeed;
    }

    convertRotationSpeedFromTuyaToHomeKit(value) {
        // Tuya values may be numeric or string ("1" vs 1); normalise both sides
        // so the reverse lookup works regardless of how the device reports state.
        let hkSpeed = this.getKeyByValue(this._rotationStops, value);
        if (hkSpeed === undefined) hkSpeed = this.getKeyByValue(this._rotationStops, String(value));
        if (hkSpeed === undefined && isFinite(value)) hkSpeed = this.getKeyByValue(this._rotationStops, Number(value));
        const result = hkSpeed === undefined ? 0 : Number(hkSpeed);
        this.log.debug('convertRotationSpeedFromTuyaToHomeKit: %s → %s', value, result);
        return result;
    }
