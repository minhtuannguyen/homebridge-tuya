...
class AirPurifierAccessory extends BaseAccessory {
    ...
    setRotationSpeed(value, callback) {
        ...
        if (value === 0) {
            ...
            // Clean mapping: HomeKit 0–33 → "1", 34–66 → "2", 67–100 → "3"
            let hkValue = Number(value);
            let tuyaSpeed = "1";
            if (hkValue > 66) tuyaSpeed = "3";
            else if (hkValue > 33) tuyaSpeed = "2";
            // Always set power on (DP 1: true) and send speed as string
            const newState = { '1': true, '4': tuyaSpeed };
            this.log.debug('setRotationSpeed sending payload:', newState);
    ...
    convertRotationSpeedFromHomeKitToTuya(value) {
     ...
     convertRotationSpeedFromTuyaToHomeKit(value) {
         // Clean mapping: Tuya "1" → 33, "2" → 66, "3" → 100
         let hkSpeed = 0;
         if (String(value) === "1") hkSpeed = 33;
         else if (String(value) === "2") hkSpeed = 66;
         else if (String(value) === "3") hkSpeed = 100;
         this.log.debug('convertRotationSpeedFromTuyaToHomeKit: %s → %s', value, hkSpeed);
         return hkSpeed;
     }
