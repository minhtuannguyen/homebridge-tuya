...
class TuyaDiscovery extends EventEmitter {
    ...
    _onDgramMessage(port, msg, info) {
        ...
        const debug = (this.log && this.log.debug) ? this.log.debug.bind(this.log) : () => {};
      //  this.log.info(`Discovery - UDP from ${info.address}:${port} 0x${msg.readUInt32BE(0).toString(16).padStart(8, '0')}...0x${msg.readUInt32BE(len - 4).toString(16).padStart(8, '0')}`);
        if (len < 16 ||
            ...
            debug(`Discovery - UDP from ${info.address}:${port}`, msg.toString('hex'));
            return;
        }

        const size = msg.readUInt32BE(12);
        if (len - size < 8) {
            debug(`Discovery - UDP from ${info.address}:${port} size ${len - size}`);
            return;
        }
        ...
        try {
            ...
            else debug(`Discovery - UDP from ${info.address}:${port} decrypted`, cleanMsg.toString('hex'));
        } catch (ex) {
            // These messages are common (encrypted broadcasts on 6667 we don't have a key for) and benign.
            debug(`Discovery - Failed to parse discovery response on port ${port}: ${decryptedMsg}`);
            debug(`Discovery - Failed to parse discovery raw message on port ${port}: ${msg.toString('hex')}`);
        }
    }
